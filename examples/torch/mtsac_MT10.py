...
def mt10_mtsac_normalize_reward(ctxt=None, seed=1, use_gpu=False, _gpu=0):
    ...
    mt10_train_envs = MultiEnvWrapper([
        normalize(GarageEnv(MT10.from_task(task_name)), normalize_reward=True)
        for task_name in task_names
                                      ...
                                      has_name=True)
    mt10_test_envs = [
        GarageEnv(MT10.from_task(task_name)) for task_name in task_names
    ]
    ...
    replay_buffer = SimpleReplayBuffer(env_spec=mt10_train_envs.spec,
